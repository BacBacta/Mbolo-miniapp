...
L'app parle sept langues : français, anglais, espagnol, portugais, swahili, russe et ukrainien. La langue affichée vient, dans cet ordre : du choix fait **dès l'accueil** ou dans **Profil → Langue de l'app**, sinon de la langue du Telegram de la personne, sinon du français.
...
**Deux cas demandent plus qu'un fichier**, et le russe les a tous les deux rencontrés :

- **Plus de deux formes plurielles.** `tn()` choisit entre la forme « un » et la forme « plusieurs » selon la règle du français. Une langue qui en a davantage — le russe et l'ukrainien en ont trois, l'arabe six — range ses variantes dans un **objet** dont les clés sont les catégories d'`Intl.PluralRules` (`one`, `few`, `many`, `other`) ; c'est alors cet objet qui décide. Fais pointer **les deux clés françaises vers le même objet** (une constante partagée en haut du fichier) : autrement, l'une des deux finira corrigée sans l'autre, et la phrase changera selon le nombre.
- **Un alphabet que Fraunces ne couvre pas.** La police des titres est latine. Pour le cyrillique, `styles.css` pose Playfair Display sur `:lang(ru)` et `:lang(uk)`, et `public/i18n.js` ne la télécharge que pour ces langues-là. Pour un autre alphabet, ajoute la langue à `DISPLAY_CYRILLIQUE` (ou à un ensemble équivalent) plutôt que de charger la police pour tout le monde — un test refuse qu'elle parte chez qui ne la verra jamais.

**L'arabe reste un autre métier** : `styles.css` porte des propriétés de direction en dur et le geste de balayage s'inverse. C'est une mise en page à refaire, pas un dictionnaire à écrire.
