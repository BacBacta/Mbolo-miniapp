...
| Découverte | Profils vérifiés de la même zone et de la même intention, dans la tranche d'âge que tu choisis, 20 « J'aime » par jour, les profils passés ne comptant pas, ceux de ton quartier d'abord (sans jamais demander ta position). Badge « Nouveau » la première semaine. Une vue **Liste** montre tous les profils compatibles, balayés ou non, avec leur statut (aimé, passé, match) et des vignettes chargées à l'apparition ; la parcourir ne consomme rien, seul un « J'aime » compte, et un « Passer » peut y être rattrapé. **Photo d'abord** (18 septembre 2026) : la carte remplit l'écran, la question et la confiance sont posées sur la photo, un chevron ouvre la fiche entière ; trois boutons ronds sous la carte — revenir, passer, aimer — et **revenir** remet la carte qu'on vient de balayer, dans la minute et jamais après un match. Le quota est une pastille dans la barre. Sans pass, le mode Liste ouvre une feuille du bas qui dit ce qu'il y a derrière. **La fiche est une suite de blocs** (lot 2) : la photo, la question dans sa propre carte, une autre photo, une autre question, les faits en bref, la confiance — au lieu de la carte du paquet en plus long ; les photos après la première ne se chargent qu'en apparaissant. **Aimer une réponse, avec un mot** : sur la fiche, chaque réponse porte un cœur ; il ouvre une feuille avec un mot facultatif de 60 caractères, passé par l'anti-arnaque comme un message. Personne ne le lit avant le match ; au match, il devient le premier message de la discussion, l'écran de match dit « {prénom} a aimé ta réponse à … », et la carte d'ouverture aussi. Il part avec le « J'aime » (revenir) et avec le compte |
| Match et discussion | **Deux coches** (pris, puis lu par l'autre), **« En ligne »** quand l'autre a la discussion ouverte, **« écrit… »** en bulle de trois points et dans l'en-tête — sur les deux chemins, flux et interrogation. Jamais « vu à telle heure ». Mesure et raisons : `audit/14-fluidite.md`. En tête de Messages, ceux qui ont aimé ton profil et attendent ta réponse (visibles quel que soit leur âge). La liste marque « à toi » quand c'est à toi de répondre. Discussion plein écran, en-tête qui ouvre la fiche de la personne, heure des messages, compteur de non lus, pseudos Telegram jamais montrés. **Le message s'affiche dès l'appui**, marqué d'une horloge tant que le serveur n'a pas confirmé — et il disparaît en rendant le texte si l'anti-arnaque le refuse. **« … écrit »** pendant que l'autre tape : signal éphémère, gardé quelques secondes en mémoire, jamais enregistré. Le rythme d'interrogation suit l'activité du fil. **Une discussion vide propose des amorces** tirées de la fiche de l'autre : elles remplissent le champ, jamais elles n'envoient à ta place. **Les messages arrivent en temps réel** par un flux ouvert avec la discussion ; si le réseau ne le laisse pas passer, l'app continue comme avant. **Appui long sur un message** : **répondre** (la citation s'affiche dans la bulle, la toucher remonte au message), et **retirer le sien** (« Message supprimé » des deux côtés ; le texte reste lisible par la modération si la discussion est signalée, jusqu'à ce que le match soit défait). **Envoyer une photo** depuis le champ : compressée sur le téléphone, montrée en aperçu léger, **voilée chez l'autre** jusqu'à ce qu'il choisisse de la voir, visible des deux membres et de personne d'autre, effacée avec le match. Rien d'autre qu'une image : ni fichier ni document. **Avant l'envoi, un aperçu et une légende** (lot 4). **Réagir à un message** : appui long, six emoji au-dessus du menu, une réaction par personne et par message, visible des deux côtés, jamais notifiée. La ligne « Liens et numéros à 10 · 3/10 » est sous l'en-tête et disparaît au seuil. « Proposer un rendez-vous » n'apparaît que s'il existe un lieu partenaire dans ta ville. **Le texte suit la taille de police de ton téléphone** (lot 5) |
...
| Jauge de confiance | Les pastilles sous chaque prénom disent ce qui est **vérifié**, et un appui explique quoi — depuis la carte elle-même, plus par un écran intercalé à l'inscription (lot 3 de l'audit 15). Elle ne compte que des critères **ouverts** : le selfie validé par la modération, et l'ancienneté de trois mois. Elle affichait « sur 3 » alors que le troisième, le garant, était figé à *non* pour tout compte réel — une jauge que personne ne pouvait remplir. Le garant ne la rejoindra pas : l'idée est abandonnée, parce que nommer un répondant laisserait croire à un recours qui n'existe pas. Ajouter un critère reste une ligne dans `server/jauge.js` le jour où un vrai mécanisme existe, et le dénominateur suivra : la carte, le score et l'écran d'explication lisent tous la même liste. L'explication s'ouvre une fois juste après le profil, puis reste dans l'onglet Profil |
...
`npm run mesure` lance le **banc de performance** (`e2e/banc.spec.js`) : Chromium à la taille d'un téléphone, processeur bridé ×4, réseau à 1,6 Mb/s et 150 ms de latence. Il mesure le chargement à froid et à chaud, le changement d'onglet, la durée des images pendant un balayage, la discussion entre deux navigateurs (frappe, message, accusé de lecture) et l'API, puis affiche un résumé et écrit `.mesure.json`. Il ne tourne pas dans `npm run e2e` : ses chiffres dépendent de la machine. Les chiffres de référence, et ce qu'ils ont fait corriger, sont dans `audit/14-fluidite.md`.
...
PLUS_SANS_VENTE_PAYS=afrique            où le pass n'est pas vendu : codes ISO, ou « afrique »
```

**Où le pass se vend, et où il est offert.** Depuis le 19 septembre 2026, décision du propriétaire, **le pass n'est pas vendu dans les pays d'Afrique** (`PLUS_SANS_VENTE_PAYS=afrique` dans `fly.toml` : les 54 États et le Sahara occidental, liste `AFRIQUE` dans `server/geo.js` ; des codes ISO séparés par des virgules font aussi l'affaire). Là, **ce que le pass ouvre est offert à tout le monde** — qui t'a aimé, qui s'est arrêté sur ta fiche, la vue Liste, tout le pays, six photos, trente secondes de voix, trois questions, la langue parlée, l'ordre du paquet — **sauf le quota de « J'aime » par jour**, qui est d'abord un frein contre les faux comptes et garde ses marches (2 sans badge, 5 avec, sans limite avec un vrai pass). Un seul endroit tranche, `passEnVente()` dans `server/plus.js`, à côté d'`estPlus()` ; `droitsOuverts()` les combine et c'est elle que lisent les portes. Le serveur refuse la vente, pas seulement l'écran : `POST /api/plus/facture` répond 403 `PASS_INDISPONIBLE`, et le bot refuse au `pre_checkout_query` une facture ouverte avant un changement de pays. L'interface ne recopie pas la règle : `GET /api/me` porte `options.passEnVente`, et sans lui aucune porte, aucune feuille, aucune ligne des réglages ni l'écran du pass n'apparaît. Un pass déjà posé (offert par `/pass`) garde son écran et sa date. Les conditions d'utilisation le disent, entre marqueurs `SI_PASS_*` résolus par `selonLaPolitique()`. **Le pays est celui déclaré sur le profil** : ni le fuseau, qui n'est pas stocké, ni une géolocalisation. C'est falsifiable — déclarer Yaoundé depuis Bruxelles donne le pass gratuit — et le sens du risque est bénin. `npm run chiffres` prévient que `pass_refuse`, `pass_vu` et `pass_usage` ne comptent rien dans ces pays : une demande absente n'y est pas une absence de demande. `test/pass-par-pays.test.js`.
