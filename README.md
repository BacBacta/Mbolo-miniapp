...
| Limitation de débit | Par compte et par action : 20 messages et 60 balayages par minute, 5 vérifications, 5 signalements, 12 photos, 10 rendez-vous et 20 modifications de profil par heure. Réponse 429 avec `Retry-After`. Compteurs en mémoire, donc remis à zéro au redémarrage et non partagés entre instances : suffisant tant que le stockage est un fichier JSON, donc mono-instance (`server/limites.js`) |
| Rendez-vous sûr | Là où des lieux partenaires existent. Ailleurs, l'app le dit au lieu d'afficher une liste vide, et rappelle de prévenir une personne de confiance. Une proposition s'**accepte ou se refuse** : la personne invitée répond, celle qui propose peut retirer sa proposition, et une fois le rendez-vous accepté **chacun peut se décommander** — se retirer d'une rencontre ne doit jamais être bloqué. Le bot prévient à chaque changement. **L'arrivée ne se confirme que sur un rendez-vous accepté**, par le scanner QR natif de Telegram : jusqu'ici on pouvait confirmer son arrivée à un rendez-vous que l'autre n'avait jamais accepté. Un seul rendez-vous vivant à la fois par discussion. Un blocage ferme aussi le rendez-vous. Tu sais que l'autre est arrivée, jamais depuis quand |
...
| Mesure produit | Six horodatages d'entonnoir dans l'objet utilisateur (`profileSavedAt`, `verificationSentAt`, `verifDecidedAt`, `firstLikeAt`, `firstMatchAt`, `firstMessageAt`), posés dans des écritures qui existaient déjà, et une table `events` : un identifiant déjà stocké, un nom d'événement, une date, une charge utile de nombres ou de mots-clés fermés. **Aucun texte** — ni message, ni prénom, ni quartier, ni champ de profil. `DELETE /api/me` emporte tout, `EVENTS_RETENTION_DAYS` (180 jours par défaut, `0` désactive l'écriture) purge le reste. Le marqueur `devUser` distingue les comptes de test, sans quoi aucun chiffre de la bêta ne serait défendable. Voir `audit/05-mesure-produit.md` |
