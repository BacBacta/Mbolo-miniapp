...
public/
  ...
  i18n/ru.js    Dictionnaire russe ; les phrases comptées y donnent leurs quatre formes
  i18n/uk.js    Dictionnaire ukrainien ; idem
...
| Langues | Français, anglais, russe et ukrainien. Choix explicite dans le profil, sinon la langue du Telegram, sinon le français. Interface traduite chez la personne (`public/i18n.js` + `public/i18n/<code>.js`, chargés à la demande ; `tn()` passe par `Intl.PluralRules`, et une phrase comptée se traduit par un objet quand la langue a plus de deux formes), messages du bot traduits côté serveur (`server/i18n.js`) dans la langue de **qui reçoit**. `PUT /api/me/lang` |
...
9. ~~**Anglais**~~ : fait, fichiers de traduction chargés à la demande, langue de Telegram par défaut, choix dans le profil, bot traduit dans la langue de qui reçoit. `antiscam.js` est également ouvert à tous les pays. **Le russe et l'ukrainien s'y ajoutent** : ce sont les deux premières langues à quatre formes de pluriel, donc `tn()` passe par `Intl.PluralRules` et une phrase comptée se traduit par un objet. Quatre contrôles de `test/langues.test.js` s'appliquent désormais à chaque langue déclarée, dont un qui refuse qu'une traduction perde une `{variable}`. **Ce que ces deux langues n'étendent pas** : `antiscam.js`, qui ne lit le vocabulaire de l'argent qu'en français et en anglais, et la modération, qui doit pouvoir relire un fil signalé. Reste à faire : le **pidgin** (`public/i18n/pcm.js` + `server/i18n.js`).
