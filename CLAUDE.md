...
test/
  ...
  bannissement, deploiement, filters, geographie, jauge, langues, limites, moderation,
...
| Jauge de confiance | `server/jauge.js` : une **liste unique** de critères ouverts (selfie validé, trois mois d'ancienneté), lue par la carte, par le score et par l'écran d'explication. Le garant n'y est pas : il n'a pas encore de mécanisme (P1-6), et une jauge « sur 3 » dont le 3 est inatteignable n'explique rien. Un critère inconnu de la liste ne compte ni au numérateur ni au dénominateur. L'écran s'ouvre une fois après l'enregistrement du profil (`localStorage`, clé `jauge_vue`), puis depuis l'onglet Profil |
...
4. **Le check-in est falsifiable, mais il n'y a rien à falsifier pour l'instant.** Le code d'un lieu est **devinable** (`rdv:lieu:palmier` se déduit de l'identifiant) : qui connaît un lieu peut confirmer une arrivée depuis chez lui. Sans conséquence tant que la liste des lieux est vide — mais **à corriger avant le premier partenariat**, et impérativement avant de facturer un lieu au rendez-vous confirmé (P2-1), sinon on facture une rencontre qui n'a pas eu lieu. Les pistes : un secret non devinable plus une fenêtre horaire (marche avec un QR sur papier), ou un code tournant affiché par le lieu (infalsifiable, demande un écran). Les lieux en base et la rotation sont prévus dans P1-10.
