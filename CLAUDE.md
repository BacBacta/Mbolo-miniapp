...
| Découverte | Même zone de recherche et même intention, 20 profils par jour, ceux qui t'ont liké en premier, économie de data (photos à la demande). **Le genre recherché ne se règle qu'en Amitié** (`gender` dans les filtres, liste fermée) : en « Relation sérieuse », `MATCH_POLICY` a déjà décidé de la mise en relation, et offrir le choix reviendrait à enregistrer l'orientation de chacun (règle 5.2). L'écran y dit la règle au lieu de proposer un réglage, le serveur n'y range rien, et quitter l'Amitié efface le champ — comme les réponses de compatibilité |
