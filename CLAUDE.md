...
test/
  ...
  l'attente, identifiants encodés, paramètres de lancement bornés),
...
| Choisir dans une liste | **Aucun `<select>` dans l'app.** Le menu déroulant d'Android n'est pas une liste de l'app : c'est une boîte de dialogue du système, à sa typographie et à ses couleurs, qu'aucune ligne de `styles.css` ne peut toucher — et **sans recherche**, ce qui faisait quarante lignes à faire défiler pour atteindre le Cameroun sur 243 pays. Le pays passe donc par un **écran à nous** (`SCREENS.pays`) : recherche en haut, pays probables proposés (celui déjà choisi, celui du fuseau, celui de la configuration), puis tout par ordre alphabétique. La recherche ignore casse, accents et ponctuation (`sansAccent()` réduit à `[a-z0-9]` : « cote divoire » trouve « Côte d'Ivoire », « guinee bissau » « Guinée-Bissau », « cm » le Cameroun par son code) et met devant ce qui **commence** par la recherche. **Elle ne reconstruit que la liste** : refaire son propre champ fermerait le clavier à chaque caractère, même cause que la règle 16. Les choix courts, eux, ne se cachent pas derrière un appui : les sept questions du profil sont des **pastilles** visibles d'un coup (`.chips`). **Pas de drapeau** dans la liste : l'emoji manque sur une partie des Android, et deux lettres dans un carré valent moins qu'un nom bien posé. `ligneDeChoix()` fabrique la ligne qui remplace un champ ; `garderLesFiltres()` met de côté ce qui est tapé dans les filtres le temps de l'aller-retour, le DOM ne survivant pas au changement d'écran (`test/interface.test.js`, `e2e/inscription.spec.js`) |
...
| Notifications bot | Match, message (limité à une par discussion toutes les 2 min), **« tu as plu à quelqu'un »** (une par jour, **sans ville** — elle donnait celle du destinataire, vraie tant que chacun cherchait dans sa ville, fausse depuis qu'un pass ouvre le pays ; et mettre celle du liker ferait un nom, croisée avec le paquet), **« quelqu'un vient d'arriver à {ville} et correspond à ce que tu cherches »**, test depuis l'onglet Profil, bouton qui rouvre le bon écran (`?screen=chat&match=`). **`onApproved` accepte plusieurs abonnés** : c'était une affectation, et un second crochet effaçait le premier en silence |
...
- Discussion par polling toutes les 4 secondes. L'ouverture d'une discussion porte un **jeton de requête** : la réponse d'une discussion quittée entre-temps est ignorée, sinon le message suivant partait à la mauvaise personne (`test/interface.test.js`).
