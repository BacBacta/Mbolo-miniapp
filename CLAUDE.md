...
public/
  ...
  i18n/es.js    Espagnol · pt.js Portugais · sw.js Swahili · ru.js Русский · uk.js Українська
                (402 phrases chacun ; ru et uk portent leurs formes plurielles en objet)
...
| Langues | **Sept langues** : français (source), anglais, espagnol, portugais, swahili, **russe et ukrainien**. **Le choix se fait dès l'accueil** (jeton en haut du héros), avant toute inscription : quelqu'un dont le Telegram est dans une langue qu'on ne connaît pas voyait du français et devait comprendre la page pour trouver le réglage qui la lui aurait rendue lisible. Il reste aussi dans l'onglet Profil. `S.langueRetour` retient d'où l'écran a été ouvert. Choix explicite, sinon la langue du Telegram, sinon le français. Chaque langue est **nommée dans sa propre langue**. Interface traduite chez la personne (`public/i18n/<code>.js`, 402 phrases, chargé à la demande), bot traduit côté serveur (`server/i18n.js`, 42 phrases) dans la langue de **qui reçoit**. **Un test compare chaque dictionnaire à l'anglais, clé par clé** : ni trou, ni clé morte. **Le russe et l'ukrainien ont demandé deux mécanismes que les trois précédentes n'avaient pas exigés.** (a) **Trois formes plurielles**, choisies sur les deux derniers chiffres et non sur la taille du nombre : 21 y repasse au singulier, 0 y prend la troisième forme. `tn()` lit donc `Intl.PluralRules` quand la traduction est un **objet** de formes ; les deux clés françaises pointent vers le **même objet**, si bien qu'aucune ne peut dériver de l'autre, et un test le vérifie. (b) **Fraunces n'a pas de glyphes cyrilliques** : sans rien, chaque titre serait tombé sur Georgia — une autre identité, au moment où l'app cherche à se faire reconnaître. `styles.css` pose Playfair Display sur `:lang(ru)` et `:lang(uk)`, et `public/i18n.js` ne la télécharge que pour ces deux langues : un test refuse qu'elle parte chez qui ne la verra jamais (règle 15). Les quatre dernières langues sont traduites **sans locuteur natif** — à faire relire avant d'ouvrir la bêta dans un pays qui les lit. L'arabe attend un chantier de mise en page (droite-à-gauche), pas un dictionnaire. `PUT /api/me/lang` |
...
- Traduction : **sept langues d'interface**, mais seuls le français et l'anglais ont été écrits par quelqu'un qui les parle. Les noms de pays viennent d'`Intl.DisplayNames` (donc traduits automatiquement), tandis que les villes, les quartiers et les textes saisis par les membres restent dans la langue de qui les a écrits — l'app ne traduit pas les gens.
...
9. ~~**Anglais**~~ : fait, fichiers de traduction chargés à la demande, langue de Telegram par défaut, choix dans le profil, bot traduit dans la langue de qui reçoit. `antiscam.js` est également ouvert à tous les pays. **Quatre langues s'y sont ajoutées** : l'espagnol (Guinée équatoriale, pays voisin), le portugais (Angola, Mozambique) et le swahili (Afrique de l'Est) — 400 phrases d'interface et 42 du bot chacune, traduites **sans locuteur natif**, donc à faire relire avant d'ouvrir dans ces pays. Reste à faire : le **pidgin** (`public/i18n/pcm.js`), dont l'orthographe n'est pas fixée et qui demande un locuteur. **L'arabe est un autre métier** : `styles.css` porte 32 propriétés de direction en dur, le geste de balayage s'inverse, et `tn()` ne connaît que singulier/pluriel quand l'arabe en a six — c'est un chantier de mise en page, pas un dictionnaire.
